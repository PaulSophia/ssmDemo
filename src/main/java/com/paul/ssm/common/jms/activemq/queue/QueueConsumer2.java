...
public class QueueConsumer2 implements MessageListener {
	...
	public static void main(String[] args) {
		QueueConsumer2 consumer = new QueueConsumer2();
		consumer.doReceive();
	}

	public void doReceive() {
		try {
			connectionFactory = new ActiveMQConnectionFactory(QueueProducer2.USERNAME, QueueProducer2.PASSWORD, QueueProducer2.BROKER_URL);
			...
			destination = session.createQueue(QueueProducer2.QUEUE_NAME);
	...
	public void onMessage(Message message) {
		...
		System.out.println("监听到MQ  QueueProducer2中有数据......");
		try {			
			System.out.println("获取MQ中数据信息   QueueProducer2  >>>>>>>>>>" +  tm.getText());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
